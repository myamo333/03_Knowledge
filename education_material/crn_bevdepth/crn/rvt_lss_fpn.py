...
class RVTLSSFPN(BaseLSSFPN):
    ...
    def get_geometry_collapsed(self, sensor2ego_mat, intrin_mat, ida_mat, bda_mat,
        ...
        points = torch.linalg.inv(ida_mat.cpu()).matmul(points.cpu().unsqueeze(-1))
        points = points.cuda()
        # cam_to_ego
        points = torch.cat(
            (points[:, :, :, :, :, :2] * points[:, :, :, :, :, 2:3],
        ...
        combine = sensor2ego_mat.cpu().matmul(torch.linalg.inv(intrin_mat.cpu()))
        ...
        if bda_mat is not None:
            ...
            points = (bda_mat @ points.float()).squeeze(-1)
        else:
